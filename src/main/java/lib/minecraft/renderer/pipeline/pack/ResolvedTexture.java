...
/**
 * The outcome of a texture resolution: the winning pack, the resolved id, and read-only byte access to
 * the PNG the caller decodes. Resolution has already applied the namespace/pack-id dispatch and the
 * within-pack root walk (last existing copy wins); the caller only decodes and memoises the bytes,
 * keyed by {@code (pack, id)}.
 *
 ...
 */
public record ResolvedTexture(@NotNull PackId pack, @NotNull ResourceId id, @NotNull PackContainer container, @NotNull String path) {
