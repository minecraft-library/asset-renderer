...
public class AppearanceOptions {
     ...
     * {@link Villager.Type#PLAINS} (default) resolves to the baked {@code type/plains} robe. Ignored by
     * entities without a villager profession layer.
     */
    private final @NotNull Villager.Type villagerType = Villager.Type.PLAINS;

    /**
     * Villager / zombie-villager profession - the clothes + hat pass over the biome robe. When set to
     * a non-{@link Villager.Profession#NONE} value and the resolved entity carries a
     * {@code texture_by: profession} overlay, that overlay draws the profession's
     * {@code <prefix>/profession/<name>} texture; {@link Villager.Profession#NONE} (default) draws no
     * profession pass. Ignored by entities without a villager profession layer.
     */
    private final @NotNull Villager.Profession villagerProfession = Villager.Profession.NONE;
     ...
     * {@link Villager.Profession#drawsBadge() draws a badge} (a real job), the resolved entity's
     * {@code texture_by: profession_level} overlay draws a tier's
     * {@code <prefix>/profession_level/<badge>} texture: the tier named here, or
     * {@link Villager.Level#minimum() the first} when empty (default), which is what vanilla clamps
     ...
     * layer.
     */
    private final @NotNull Optional<Villager.Level> villagerLevel = Optional.empty();
