 ...
 * caller wires into is a concrete implementation of {@link lib.minecraft.renderer.Renderer},
 ...
 * <ul>
 *   <li>{@link lib.minecraft.renderer.AtlasRenderer} - bulk render every block / item / entity
 *       in a {@link lib.minecraft.renderer.engine.RendererContext} into a single grid image
 *       plus sidecar JSON describing each tile's coordinates.</li>
 *   <li>{@link lib.minecraft.renderer.BlockRenderer} - vanilla block models composed into
 ...
 *   <li>{@link lib.minecraft.renderer.EntityRenderer} - mob entities driven by the Java pipeline
 *       ({@code entity_models.json} / {@code entity_geometry.json}, produced by
 *       {@link lib.minecraft.renderer.tooling.ToolingEntityModels}). The largest renderer in
 ...
 *   <li>{@link lib.minecraft.renderer.FluidRenderer} - water / lava in isometric 3D (sloped
 *       top, flow-rotated UVs, animation) or as a flat source-face icon.</li>
 *   <li>{@link lib.minecraft.renderer.GridRenderer} - compose a rectangular grid of tiles, each
 *       a static PNG or animated WebP, into one output via
 *       {@link lib.minecraft.renderer.kit.FrameMerger}.</li>
 *   <li>{@link lib.minecraft.renderer.ItemRenderer} - vanilla item models with all the
 ...
 *   <li>{@link lib.minecraft.renderer.LayoutRenderer} - free-form composition of child
 *       renderers (or pre-rendered images) into a single canvas via a
 *       {@link lib.minecraft.renderer.options.Layout} strategy.</li>
 *   <li>{@link lib.minecraft.renderer.MenuRenderer} - inventory-style screens (player, chest,
 *       crafting table, anvil) with the vanilla theme chrome and per-slot item icons.</li>
 *   <li>{@link lib.minecraft.renderer.PlayerRenderer} - player skin renders at three body
 *       scopes ({@code SKULL}, {@code BUST}, {@code FULL}) and two perspectives, with optional
 *       armor and trim layers.</li>
 *   <li>{@link lib.minecraft.renderer.PortalRenderer} - end portal and end gateway, both
 *       reproducing vanilla's CPU-baked parallax star-field shader.</li>
 *   <li>{@link lib.minecraft.renderer.TextRenderer} - styled Minecraft text in lore-tooltip
 ...
 * </ul>
 *
 * <p><b>Common defaults.</b> {@link lib.minecraft.renderer.Renderer#DEFAULT_OUTPUT_SIZE} is the
