 ...
 * {@link RenderEngine#relightForItems3d} pass and the shared rasterizer handle culling and
...
public class ShieldKit {
     ...
     * pose, scale, and translation and runs the {@link RenderEngine#relightForItems3d} pass.
     *
     ...
     * ({@link RenderEngine#computeItemsFlatLighting} vs the {@code ITEMS_3D} variant). Measured
    ...
    public static @NotNull ConcurrentList<VisibleTriangle> relightShield(
        ...
        for (VisibleTriangle t : triangles) {
            ...
            // Match vanilla's signed-byte SNORM normal round-trip (see RenderEngine.packAsSnormByte).
            Vector3f packed = new Vector3f(snorm(rendered.x()), snorm(rendered.y()), snorm(rendered.z()));
            float shading = RenderEngine.computeItemsFlatLighting(packed);
