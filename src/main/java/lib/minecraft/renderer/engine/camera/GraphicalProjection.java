 ...
 * camera {@link Camera pose} and {@link Projection flatten}. Each constant bundles a canonical base
 ...
 * {@link Camera} / {@link Projection} objects and ignore facing.
 *
 ...
 */
public enum GraphicalProjection {
     ...
     */
    ONE_POINT(new EulerRotation(0f, 180f, 0f), Projection.perspective(0.6f, 8f, 8f, 0.45f)),
     ...
     */
    TWO_POINT(new EulerRotation(0f, 215f, 0f), Projection.perspective(0.6f, 8f, 8f, 0.45f)),
     ...
     */
    THREE_POINT(new EulerRotation(30f, 215f, 0f), Projection.perspective(0.6f, 8f, 8f, 0.45f)),
     ...
     */
    ISOMETRIC(new EulerRotation(35.264f, 225f, 0f), Projection.orthographic(0.45f)),
     ...
     */
    DIMETRIC(new EulerRotation(26.565f, 225f, 0f), Projection.orthographic(0.5f)),
     ...
     */
    TRIMETRIC(new EulerRotation(20f, 250f, 0f), Projection.orthographic(0.5f)),
     ...
     */
    CAVALIER(new EulerRotation(0f, 180f, 0f), Projection.oblique(1.0f, (float) Math.toRadians(-45), 0.5f)),
     ...
     */
    CABINET(new EulerRotation(0f, 180f, 0f), Projection.oblique(0.5f, (float) Math.toRadians(-45), 0.5f)),
     ...
     */
    MILITARY(new EulerRotation(90f, 225f, 0f), Projection.oblique(1.0f, (float) Math.toRadians(-45), 0.5f)),
     ...
     */
    VANILLA_BLOCK(EulerRotation.STANDARD_ISO_BLOCK, Projection.ISOMETRIC_BLOCK),
     ...
     */
    VANILLA_PLAYER(EulerRotation.STANDARD_ISO_PLAYER, Projection.NONE),
     ...
     * the default.
     */
    VANILLA_GUI_ITEM(EulerRotation.NONE, Projection.GUI_ITEM);

    /**
     * Resolved camera pose, flatten, and lighting pose for one {@link GraphicalProjection} at a chosen
     ...
     */
    public record Resolved(@NotNull Camera camera, @NotNull Projection flatten, @NotNull EulerRotation lightingPose) {}

    private final @NotNull EulerRotation basePose;
    private final @NotNull Projection baseFlatten;

    GraphicalProjection(@NotNull EulerRotation basePose, @NotNull Projection baseFlatten) {
    ...
    public @NotNull Resolved resolve(@NotNull HorizontalFacing horizontal, @NotNull VerticalFacing vertical) {
        return switch (this) {
            case VANILLA_BLOCK ->
                new Resolved(Camera.forBlockIcon(), Projection.ISOMETRIC_BLOCK, EulerRotation.STANDARD_ISO_BLOCK);
            case VANILLA_PLAYER ->
                new Resolved(Camera.forPlayerIcon(), Projection.NONE, EulerRotation.STANDARD_ISO_PLAYER);
            case VANILLA_GUI_ITEM ->
                new Resolved(Camera.identity(), Projection.GUI_ITEM, EulerRotation.NONE);
            default -> {
                EulerRotation pose = facePose(this.basePose, horizontal, vertical);
                Projection flatten = this.baseFlatten.kind() == Projection.Kind.OBLIQUE
     ...
     */
    private static @NotNull Projection faceOblique(@NotNull Projection base, @NotNull HorizontalFacing h, @NotNull VerticalFacing v) {
        ...
        return Projection.oblique(base.obliqueDepthFactor(), (float) Math.atan2(sin, cos), base.projectionScale());
    }
