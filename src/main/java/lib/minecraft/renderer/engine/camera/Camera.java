 ...
 *
 * <p>Lighting is <b>not</b> carried here - it is a separate {@link LightingFrame} paired with the
 * camera in a {@link View}, so a subject can be posed one way and lit another. {@link Projection#resolve}
 * produces the pair, defaulting the light to track the pose.
 *
...
public record Camera(@NotNull Matrix4f pose, @NotNull Lens lens) {
     ...
     * Euler-angle rotation, paired with the given lens. Use it for every {@link Projection} display-pose
     ...
     * projection's resolved camera under a different flatten - e.g. a 2-block bed icon that wants the
     * {@link Projection#VANILLA_ISO} pose but a more conservative lens for margin - without rebuilding
     * the pose or reaching for the raw constructor.
     *
