 ...
 * <p>
 * A call to {@link #sampleFrame(PixelBuffer, MCMeta.Animation, int) sampleFrame} returns the frame
 * for a specific tick, honoring the animation's per-entry duration overrides and optional linear
 * interpolation between adjacent frames. Each entry lasts {@link MCMeta.Frame#time()}
 * ticks, falling back to {@link MCMeta.Animation#frametime() frametime} whenever the entry declares
 * no positive override. When {@link MCMeta.Animation#interpolate()} is {@code true}, the result is a
...
public class AnimationKit {
     ...
     * {@link MCMeta.Frame} contributes its {@code time} ticks, or {@code frametime}
     * when the entry declares no positive override. Out-of-range entry indices are clamped into
     * {@code 0..frameCount-1}.
     * <p>
     * When {@link MCMeta.Animation#interpolate() interpolate} is set, the returned frame is a linear
    ...
    public static @NotNull PixelBuffer sampleFrame(
        ...
        @NotNull MCMeta.Animation animation,
        int tick
    ) {
        ...
        if (animation.frames().isEmpty()) {
            ...
            for (int i = 0; i < size; i++) {
                MCMeta.Frame entry = animation.frames().get(i);
     ...
     * implicit frames {@code 0..frameCount-1} each last {@link MCMeta.Animation#frametime() frametime}
     * (floored at 1); otherwise each {@link MCMeta.Frame} contributes its {@code time}
     ...
     */
    public static int @NotNull [] entryDurations(int frameCount, @NotNull MCMeta.Animation animation) {
        int defaultTicks = Math.max(1, animation.frametime());
        if (animation.frames().isEmpty()) {
     ...
     */
    public static int frameWidth(@NotNull PixelBuffer strip, @NotNull MCMeta.Animation animation) {
     ...
     */
    public static int frameHeight(@NotNull PixelBuffer strip, @NotNull MCMeta.Animation animation) {
