 ...
 * ({@link FloatVector#SPECIES_128}, 4-lane) to compute the three output components in a
 * single horizontal accumulation instead of three independent scalar dot products.
 * <p>
 * The operation order matches the scalar implementation exactly - no fused multiply-add is
 * used, and adds are accumulated left-to-right against the row vectors in the same sequence
 * the scalar code writes them - so results are bit-identical per IEEE-754 round-to-nearest-even.
 * This is deliberate: the existing regression-pin CRC32s in
 * {@code ModelEngineParallelismTest}, {@code PortalRendererParallelismTest}, and
 * {@code FluidRendererParallelismTest} stay valid after this class is wired in.
 * <p>
 * Callers outside the per-vertex hot path (scene-setup code, diagnostic tools) should keep
 * using {@link Vector3f#transform} / {@link Vector3f#transformNormal} - the SIMD overhead per
 * call swamps the benefit when transforms happen once or twice per render.
 */
@UtilityClass
public class Vector3fOps {
    ...
    /**
     * Transforms {@code v} as a point ({@code w=1}) by {@code m} and returns the result.
     * Bit-identical to {@link Vector3f#transform(Vector3f, Matrix4f)}; faster on vertex-heavy
     * workloads where the method is called thousands of times per render.
     *
    ...
    public static @NotNull Vector3f transform(@NotNull Vector3f v, @NotNull Matrix4f m) {
        ...
        // Per-lane: ((v.x*m{1,j} + v.y*m{2,j}) + v.z*m{3,j}) + m{4,j}.
        // Lane 0 yields tx, lane 1 yields ty, lane 2 yields tz. Lane 3 (tw) is computed too
        // but discarded - harmless, and avoids a mask load on every invocation.
    ...
    /**
     * Transforms {@code v} as a direction ({@code w=0}) by {@code m}, ignoring the translation
     ...
     * no heap pressure under sustained use. Kept out of the hot loop as a separate helper so
     * the inline SIMD arithmetic stays readable.
     */
