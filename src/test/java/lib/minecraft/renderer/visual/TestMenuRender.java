 ...
 * the chest-slot chrome can be eyeballed. Covers the two menu types that draw the gray "vanilla chest"
...
public final class TestMenuRender {
    ...
    private static java.util.Map.Entry<Integer, MenuOptions.MenuSlotContent> slot(int index, String itemId) {
        return java.util.Map.entry(index, MenuOptions.MenuSlotContent.of(itemId));
    }

    @SafeVarargs
    private static dev.simplified.collection.ConcurrentMap<Integer, MenuOptions.MenuSlotContent> slots(
        java.util.Map.Entry<Integer, MenuOptions.MenuSlotContent>... entries) {
        dev.simplified.collection.ConcurrentMap<Integer, MenuOptions.MenuSlotContent> map = dev.simplified.collection.Concurrent.newMap();
        for (java.util.Map.Entry<Integer, MenuOptions.MenuSlotContent> entry : entries)
