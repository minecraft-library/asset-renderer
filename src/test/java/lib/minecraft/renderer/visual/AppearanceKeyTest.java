...
final class AppearanceKeyTest {
    ...
    private static @org.jetbrains.annotations.NotNull Entity plain(String id) {
        return Entity.builder()
            .id(ResourceId.parse(id))
            .model(new EntityModelData())
            .axes(new Entity.Axes(Map.of(), Optional.empty(), List.of(), Optional.empty(),
                Map.of(), Map.of(), Map.of(), Optional.empty(), Optional.empty(), Optional.empty()))
            .build();
    }

    /** A model carrying a variant axis with the given options and declared default. */
    private static @org.jetbrains.annotations.NotNull Entity variantModel(
        String id, String defaultOption, String... options) {
        Map<String, Entity> coats = new java.util.LinkedHashMap<>();
        for (String option : options) coats.put(option, plain(id));
        return Entity.builder()
            .id(ResourceId.parse(id))
            .model(new EntityModelData())
            .axes(new Entity.Axes(Map.of(), Optional.empty(), List.of(), Optional.empty(),
                Map.of(), Map.of(), Map.copyOf(coats), Optional.of(defaultOption),
                Optional.empty(), Optional.empty()))
            .build();
    }

    /** A model carrying a size axis with a declared default. */
    private static @org.jetbrains.annotations.NotNull Entity sizedModel(String id, String defaultSize) {
        return Entity.builder()
            .id(ResourceId.parse(id))
            .model(new EntityModelData())
            .axes(new Entity.Axes(Map.of(), Optional.empty(), List.of(), Optional.empty(),
                Map.of(), Map.of(), Map.of(), Optional.empty(), Optional.empty(), Optional.of(defaultSize)))
            .build();
    }
