...
/**
 * Exercises {@link PipelineRendererContext#resolveArmorTextureOverride}: a synthetic {@code type=armor}
 * / {@code type=elytra} CIT rule retextures a matching equipped piece, a non-matching context and a
 * vanilla-only stack resolve to {@link CitResult#NONE}, and the layer type selects the rule subject
 * ({@link LayerType#WINGS} wants {@code type=elytra}, any other {@code type=armor}).
 */
@DisplayName("PipelineRendererContext.resolveArmorTextureOverride walks CIT armor / elytra rules")
class ResolveArmorTextureOverrideTest {
