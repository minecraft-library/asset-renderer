 ...
 *
 * <p>The trigonometry pair is deliberately the same assertion the tooling build makes about its own
 * copy of the table: both re-declare the index-per-radian constant rather than importing one, and
 * both rebuild the sampled value from {@link Math#sin(double)} at the resolved index. Two builds
 * that cannot see each other therefore hold one contract, and a drift in either fails on its own
 * side rather than surfacing as a moved pixel.
 *
