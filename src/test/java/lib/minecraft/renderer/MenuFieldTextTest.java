...
class MenuFieldTextTest {
    ...
    private static final String OVER_LENGTH = "x".repeat(Decoration.Field.MAX_LENGTH + 12);
    ...
    void textPastTheCapIsDropped() {
        assertThat(MenuRenderer.typedInto(MenuOptions.builder().fieldText(OVER_LENGTH).build()).length(),
            is(equalTo(Decoration.Field.MAX_LENGTH)));
        assertThat("a name within the cap arrives whole",
            MenuRenderer.typedInto(MenuOptions.builder().fieldText("Excalibur").build()),
            is(equalTo("Excalibur")));
        assertThat("and the parity subjects' own name sits exactly on it",
            AT_LENGTH.length(), is(equalTo(Decoration.Field.MAX_LENGTH)));
    }

    @Test
    @DisplayName("a name that fits is shown whole")
    void aNameThatFitsIsShownWhole() {
        assertThat(MenuRenderer.visibleTail("Excalibur"), is(equalTo("Excalibur")));
        assertThat(MenuRenderer.visibleTail(""), is(equalTo("")));
    }
    ...
    void aNameTooWideIsShownFromItsEnd() {
        assertThat("the fixture is wider than the field", widthOf(AT_LENGTH),
            is(greaterThan(Decoration.Field.INNER_WIDTH)));

        String tail = MenuRenderer.visibleTail(AT_LENGTH);
        ...
        assertThat("it fits", widthOf(tail), is(lessThanOrEqualTo(Decoration.Field.INNER_WIDTH)));
    ...
    void theAnvilIsTheOneScreenThatDeclaresAField() {
        for (MenuScreen screen : MenuScreen.measured()) {
            boolean hasField = screen.layout(true).decorations().stream()
                .anyMatch(Decoration.Field.class::isInstance);
    ...
    void theFieldsTextSitsInsideItsOwnWell() {
        ...
        Decoration.Field field = layout.decorations().stream()
        ...
        assertThat("the text opens clear of the bevel", Decoration.Field.TEXT_INSET_X, is(greaterThan(2)));
        assertThat("the run fits between the bevels",
            Decoration.Field.TEXT_INSET_X + Decoration.Field.INNER_WIDTH,
            is(lessThanOrEqualTo(field.extent().width() - 2)));
    }
