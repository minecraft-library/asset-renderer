...
- **A coplanar pair is decided by emission order, last-drawn-wins** (`depthFails` passes an equal depth - vanilla `GL_LEQUAL`). `EntityModelData.getBones()` order therefore *is* the tied-depth priority, which is why Pass 1 preserves encounter order and non-translucent triangles are never sorted. That order is the tooling's `addOrReplaceChild` declaration order, **not** vanilla's `PartDefinition.children` `HashMap` bake order - and reproducing the bake order was built, measured and **declined**, because vanilla does not decide these pixels by draw order either. Harvesting every bit-identical-depth contest and asking the reference who won shows no order can match: `right_arm` beats `torso` on 12 of 13 armoured wearers (which the bake order forbids) while `body:east` beats `left_arm:west` on the bare bodies (which declaration order forbids), and the same shared armour mesh resolves its boot seam to `left_leg` on seven wearers and `right_leg` on the other seven - usually unanimously within a wearer (piglin `368/0`, skeleton `0/214`), which is a systematic ULP difference, not an order. It is the GPU shader-float floor, and post-L26 the Java side has none of that noise by construction, so it picks one answer and holds it for every wearer. **Do not re-open this by reordering bones** - the measurement is frozen in `notes/refharness-unify/baselines/probe-L27-bake-order*.tsv`.
...
- Blocks/items are structurally immune (unit cube at fixed scale, no coplanar sibling boxes): a depth change that moves entities leaves **0 of 1055** block and **0 of 479** item rows. Use that as the discriminator when a rasterizer change moves something unexpected.
