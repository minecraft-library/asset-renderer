...
- `entityModels` -> `entity_models.json` + `entity_geometry.json` + `entity_armor.json` (ASM scan of vanilla client jar). Entry: `ToolingEntityModels.main` -> `EntityToolingContext.of(jar)` -> per-entity resolver fan-out (see `tooling/entity/`). `entity_models.json` is the **normalized model form** (see below); `EntityRuntimeJsonWriter` builds the flat per-entity tables in-memory and `EntityFamilyJsonWriter` groups them into it. `entity_armor.json` is the worn-armour meshes (see below).
...
`entity_models.json` is the **normalized model form**: one entry per base entity (`minecraft:wolf`, not a row per colour), keyed under a top-level `models` map - 90 models in 26.1. Each model carries `geometry_ref`/`armor_type`/overlays once, plus:
...
### Worn armour (`entity_armor.json`)
Vanilla never derives worn armour from the wearer's own mesh - `LayerDefinitions` builds a handful of `ArmorModelSet`s and hands each renderer the one its subject wears, so a skeleton's narrow limbs and a giant's scaled-up body dress in the same boxes. `tooling/vanilla/ArmorMeshIndex` walks `createRoots` for those registrations and `tooling/entity/EntityArmorFlow` parses each set's base mesh factory through the shared `GeometryParser`, writing `entity_armor.json`: `meshes` (the bone trees, keyed `<SimpleClass>#<method>@inner=<g>@outer=<g>`), `sets` (armour-set name -> mesh key), `shared` (the mesh a wearer that names none is dressed in). 17 sets over **4 distinct meshes** in 26.1.
- **Each mesh is emitted twice, already grown** - once at the set's inner deformation, once at its outer - because the legs carry their own `CubeDeformation.extend` and so differ between the two by more than a constant. `ArmorKit` picks inner for leggings, outer for the other three slots, and reads the corners with no grow arithmetic.
- **Selection is data**: `EntityLayersResolver.resolveArmorMesh` records the `ArmorModelSet` field name on the `layers[]` armour row as `armor_mesh`, leaf-first along the renderer's constructor chain, falling back to the armour set among the registration lambda's `ModelLayers` references (the piglin family takes its set as a constructor argument, so no field along the chain names it). All 14 armoured entities name one.
- **Per-mesh deformations travel with the mesh**, which is what gives the piglin family its outer `1.02`. The whole-mesh `MeshTransformer.scaling` three sets are registered through (giant `6.0`, husk `1.0625`, wither skeleton `1.2`) is deliberately **not** carried: vanilla applies the same transformer to those wearers' body layers, and `EntityArmorFrame` already reads that scale off the wearer's torso bone - carrying it here would apply it twice.
