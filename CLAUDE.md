...
- **Known divergence - and a trap if you ever model `source`.** `clock.json` is the only one of 1506 vanilla item trees that selects on `context_dimension`. With that property unevaluable the renderer takes its **fallback** branch - which is not a degradation branch but the **Nether/End** one, where vanilla clocks spin randomly (`source: random`, against the overworld case's `source: daytime`). The two branches differ in that single key: all 65 entries are identical entry-for-entry at the same `scale: 64.0`. So we render a Nether clock and get the right overworld face anyway, because two errors cancel - the tables agree, **and** `source` is never parsed (`ItemModelNodeDeserializer` reads `property`/`scale`/`target`/`index`/`entries`/`fallback` only).
  Those two errors are coupled: **modelling `source` without also wiring `context_dimension` in the same change would break the clock**, resolving a genuinely random face per render, because it would finally honour the branch we were always taking. Wire `case "context_dimension" -> Optional.of("minecraft:overworld")` into `ItemModelContext.selectValue` **first** (no record component, so `gui()`/`isNeutral()` stay untouched and the pixel diff is provably zero today). Same applies if a version bump ever makes the two tables diverge.
