...
Single-purpose headless Fabric mod that drives the real MC client to produce the byte-stable ground-truth PNGs [asset-renderer]'s parity tests diff against - this mod lives inside that repo, at `harness/`, and stays its own Gradle build. Nine sweeps: **blocks** (true 3D, not item icons), **entities**, **non-block items** (GUI icons), the **player**, **armored mobs**, **animated glint**, **container screens**, **animated entities**, and **walking entities**. **README is the user-facing reference** (architecture, mixin catalog, family map, configuration); this file is the session-refresh / contributor's quick reference.

**The last two are the odd ones and each needs its own boot.** Seven of them freeze `setupAnim` and are ground truth for the mesh as authored; the two posed sweeps turn that freeze off and are ground truth for the mesh its model poses. Both freezes read `HarnessConfig.POSED` once per JVM, so a run that poses one subject poses every subject and the sets cannot share a client. **The two posed sweeps are one sweep at two gaits** - `EntityAnimationSweep` writes `animation/` at the gait a subject standing still is in and `walk/` under `-Drefharness.walking=true`, which additionally drives `walkAnimationPos` / `walkAnimationSpeed`; a subject cannot be standing still and walking in one client either, so that is a third boot rather than a second sweep. `renderVanillaAllReferences` therefore boots three times - both posed passes are ordering edges on it rather than arms of `EVERY`.
...
- Animated-only: `./gradlew runRenderReferences -PrefharnessAnimated=true`, or `./gradlew renderVanillaAnimationReferences` from asset-renderer. ~58 s for 90 subjects x 8 frames into `animation/<subject>/frame_NNN.png`.
- Walk-only: `./gradlew runRenderReferences -PrefharnessWalking=true`, or `./gradlew renderVanillaWalkReferences` from asset-renderer. The same 90 subjects at the same 8 ticks into `walk/<subject>/frame_NNN.png`, measured at **58 s**, with the stride driven at `AnimationClock.WALK_AMPLITUDE`. Together these two are the ONLY runs whose client draws a posed subject; every other one, both flags unset, is bit-for-bit what it always was - measured, not asserted: adding the gait re-rendered `animation/` byte-identical on all 720 frames.
...
- Output dirs under the output root: `blocks/`, `entities/`, `items/`, `glint/` (+ `glint/atlas_uv.json`), `armor/`, `menus/`, `animation/`, `walk/`.
...
- `FreezeAnimationStateMixin` (`LivingEntityRenderer.extractRenderState`) - zero per-tick anim fields (`deathTime`, `ticksSinceKineticHitFeedback`, `wornHeadAnimationPos`, ...); force `isInWater` on `AbstractFish` (salmon/cod/tropical_fish upright). **Three of its fields are answered by `AnimationClock` rather than pinned**: `ageInTicks` is the frame's tick on a posed run, and `walkAnimationPos` / `walkAnimationSpeed` are the stride on a walking one. They move together because vanilla accumulates the phase BY the amplitude once a tick, so one without the other is no gait.
- `SuppressShakingMixin` (`LivingEntityRenderer.setupRotations`) - cancel the `isShaking` bodyRot wobble.
- `SkipSetupAnimMixin` (every `setupAnim` callsite) - authored bind pose, not frame-0 animated pose. **Broadest freeze**, and the one `HarnessConfig.POSED` lifts - which both posed gaits set.
...
- `PhantomStateMixin` - `flapTime = getUniqueFlapTickOffset()` taken out, the schedule left in. The renderer builds the field as `offset + ageInTicks`, so the flap is a pure function of the clock and only the id-derived offset has to go; pinning the whole sum took the schedule with it and held the wings still. Inert at tick 0 by construction, so the frozen sub-trees do not see it.
- `PufferfishStateMixin` - `puffState = STATE_FULL`.
- **The `*IdleMixin` family** - `SquidIdleMixin`, `ShulkerIdleMixin`, `EnderDragonIdleMixin`, `AxolotlIdleMixin`, `DolphinIdleMixin`. Not pins: they DRIVE a field vanilla's own `tick` or `aiStep` fills and a never-ticked subject leaves at zero, so that the reference shows the animation rather than a subject frozen at a value no player ever sees. All five read `IdleFigures`, which gates on `HarnessConfig.POSED` and answers the resting value otherwise - so the seven frozen sub-trees are byte-identical and the two posed ones move. **The arithmetic is shared with the asset side by both declaring it and a test comparing the two as text**, `IdleFigureMirrorTest` over there; a value that moved on one side only renders happily and reports as a defect in that renderer. Two shapes: a scalar moved across the range vanilla's own arithmetic bounds it to, and a one-hot whose selected member answers one. Adding one is a roster line on both sides and a three-line mixin here.
...
> `HarnessConfig.POSED` and the frozen and posed reference sets exist side by side.
>
