...
- **sees** `sweep.entity`, `sweep.block`, `sweep.item`, `sweep.armor`, `sweep.glint`, `pin.player-crc`, `pin.vanilla-iso-pose`, `pin.kit-corners`, `pin.block-crc`, `pin.fluid-crc`, `pin.portal-crc`, `manifest.fluid`, `manifest.portal`, `manifest.player-raw`, `manifest.visual`, `sweep.entity-animation`
- **blind** `manifest.dump.vanilla`, `manifest.dump.packs`
...
## B61 - The animated sweep poses every subject and writes animation/ alone
...
*Probe:* change the frame count and re-render the whole tree: animation/ changes population and the seven frozen sub-trees hash identically, because what writes them is a different boot of the client
